...
class Engine(gym.Env, gym.utils.EzPickle):
    ...
    def build_layout(self):
        ...
        if not self.randomize_layout:
            self.rs = np.random.RandomState(6)
            #4,6 is good
    ...
    def sample_goal_position(self):
        ...
        robot_pos = self.layout['robot']
        if self.near_goal:
            placements = [(robot_pos[0] - self.goal_range, robot_pos[1]- self.goal_range,\
                 robot_pos[0] + self.goal_range, robot_pos[1] + self.goal_range)]
            keepout = self.goal_keepout
    ...
    def obs_lidar(self, positions, group):
        ...
        if self.lidar_type == 'pseudo':
            # by default, we use pseudo lidar
    ...
    def get_lidar_obs_with_keys(self):
        ''' Return the observation of our agent '''
        self.sim.forward()  # Needed to get sensordata correct
        obs = {}
    ...
    def get_global_obs(self):
        ...
        if self.observe_vision:
            obs['vision'] = self.obs_vision()
        obs['robot_global_pos'] = self.world.robot_pos()
        obs['robot_mat'] = self.world.robot_mat()
        return obs
    def get_ego_centric_obs(self):
        ''' Return the observation of our agent '''
        self.sim.forward()  # Needed to get sensordata correct
        obs = {}
    ...
    def get_obj_pose(self):
        if self.ego_centric:
            return self.get_ego_centric_obs()
        else:
            return self.get_global_obs()

    def get_mlp_agent_obs(self, force_apply=False):
        "the mlp here means concatenating the obj pose together as a vector"
        "it's not using the lidar observation"
        obj_pose = self.get_obj_pose()
        output = []
        for key in obj_pose.keys():
            if force_apply:
               if 'hazards_pos' in key or 'pillars_pos' in key: 
                   continue
    ...
    def get_gnn_agent_obs(self, node_type_flag=False):
        obj_pose = self.get_obj_pose()
        node_type_dict = {'robot_global_pos': 0, 'goal_pos': 1, 'hazards_pos': 2, 'pillars_pos': 3, 'box_pos': 4}
        pose_output = []
        node_type_list = []
        state_output = []
        for key in obj_pose.keys():
            # change all of the pose to two dimensional vector
            if key in node_type_dict.keys():
                if key == 'robot_global_pos':
                    if self.ego_centric:
                        data = np.zeros((1,2))
                    else:
                        data = obj_pose[key][np.newaxis, :]
                elif key == 'goal_pos' or key == 'box_pos':
                    data = obj_pose[key][np.newaxis, :]
                else:
                    data = obj_pose[key]
        ...
        pose_output = np.concatenate(pose_output)
        node_type_list = np.concatenate(node_type_list)
        state_output = np.concatenate(state_output)
        return pose_output, node_type_list, state_output
